...
class LitPcbaDataset(CacheableDataset):
    ...
    def __init__(self, cfg, target):
        ...
        self.items = [ (sm.strip(), True) for sm in actives ]
        self.items += [ (sm.strip(), False) for sm in inactives ]
    ...
    def __getitem__(self, index):
        smiles, is_active = self.items[index]
    ...
    def get_type_data(self):
        return IsActiveData.get_type_data(self.cfg)
